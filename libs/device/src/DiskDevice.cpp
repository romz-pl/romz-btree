...
struct ScopedSpinlock {
    ScopedSpinlock( const Spinlock& ) {}

};

DiskDevice::DiskDevice(const EnvConfig &config_)
    : Device(config_)
{
    DiskState state;
...
void DiskDevice::open()
{
    ...
    DiskState state = std::move(m_state);
    state.file.open(config.filename.c_str(), config.read_only);
    ...
    try {
        state.file.mmap(0, state.mapped_size, config.read_only, &state.mmapptr);
    }
...
void DiskDevice::read_page(Page *page, uint64_t address)
{
    ...
    if (address < m_state.mapped_size && m_state.mmapptr != nullptr) {
